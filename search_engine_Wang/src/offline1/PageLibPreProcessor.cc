...
namespace se
{
...
#if 0
    void PageLibPreProcessor::display()
    {
        auto it = _invertIndexLib["李克强"];
        ...
        cout << "N = " << _pageList.size() << endl;
        cout << "sum = " << _pageList[396].getSum() << endl;
        cout << "sum = " << _pageList[825].getSum() << endl;
        cout << "DF = " << _invertIndexLib["李克强"].size() << endl;
    }
#endif
    ...
    void PageLibPreProcessor::buildInvertIndexMap()
    {
        ...
        {
            for (auto it = _pageList[idx].getWordsMap().begin();
                 it != _pageList[idx].getWordsMap().end(); ++it)
            {
        ...
        for (int idx = 0; idx < _pageList.size(); ++idx)
        {
            double wSum = sqrt(_pageList[idx].getSum());
            for (auto it = _pageList[idx].getWordsMap().begin();
                 it != _pageList[idx].getWordsMap().end(); ++it)
            {
                auto it1 = _invertIndexLib[it->first].find(idx);
                it1->second /= wSum;
            }
        }
#endif
